...
function geomgenerator674377244746970594098582_eval_expression(context) {
    // difference(difference($geometry, buffer(start_point($geometry), 10000)), buffer(end_point($geometry), 10000))

    var feature = context.feature;
    
    return fnc_difference([fnc_difference([fnc__geometry([[]], context),fnc_buffer([fnc_start_point([fnc__geometry([[]], context)], context),10000], context)], context),fnc_buffer([fnc_end_point([fnc__geometry([[]], context)], context),10000], context)], context);
}


function arrow_width_dd_expression74155417458606378_eval_expression(context) {
    // scale_linear(weight, 1000, 10000, 0.8, 3) * CASE WHEN weight < 1000 THEN 0 ELSE 1 END

    var feature = context.feature;
    function _CASE() {
    if (feature.get('weight')  < 1000) {
          return 0;
        }
    else {
     return 1;
    ...
    return (fnc_scale_linear([feature.get('weight') ,1000,10000,0.8,3], context) * _CASE());
}


function head_length_dd_expression5390563927422695764782661_eval_expression(context) {
    // scale_linear(weight, 1000, 10000, 2, 6) + 1.5 * CASE WHEN weight < 1000 THEN 0 ELSE 1 END
    ...
    return (fnc_scale_linear([feature.get('weight') ,1000,10000,2,6], context) + (1.5 * _CASE()));
}


function head_thickness_dd_expression196046489053596_eval_expression(context) {
    // scale_linear(weight, 1000, 10000, 0.1, 1.5) + 1.5 * CASE WHEN weight < 1000 THEN 0 ELSE 1 END
    ...
    return (fnc_scale_linear([feature.get('weight') ,1000,10000,0.1,1.5], context) + (1.5 * _CASE()));
}


function geomgenerator7058809145244317259_eval_expression(context) {
    // buffer(end_point($geometry), 10000)

    var feature = context.feature;
    
    return fnc_buffer([fnc_end_point([fnc__geometry([[]], context)], context),10000], context);
}


function geomgenerator063256762446830280373864_eval_expression(context) {
    // buffer(start_point($geometry), 10000)

    var feature = context.feature;
    
    return fnc_buffer([fnc_start_point([fnc__geometry([[]], context)], context),10000], context);
}


function geomgenerator195139411455929428336811_eval_expression(context) {
    // difference(difference($geometry, buffer(start_point($geometry), 10000)), buffer(end_point($geometry), 10000))
...
function arrow_width_dd_expression2214555243499804050118_eval_expression(context) {
    // scale_linear(weight, 1000, 10000, 0.8, 3) * CASE WHEN weight < 1000 THEN 0 ELSE 1 END
...
function head_length_dd_expression9945709994109909802_eval_expression(context) {
    // scale_linear(weight, 1000, 10000, 2, 6) + 1.5 * CASE WHEN weight < 1000 THEN 0 ELSE 1 END
...
function head_thickness_dd_expression964216617436854871448_eval_expression(context) {
    // scale_linear(weight, 1000, 10000, 0.1, 1.5) + 1.5 * CASE WHEN weight < 1000 THEN 0 ELSE 1 END
...
function geomgenerator5378200546104036077414_eval_expression(context) {
    // buffer(end_point($geometry), 10000)
...
function geomgenerator8834468867579617_eval_expression(context) {
    // buffer(start_point($geometry), 10000)
