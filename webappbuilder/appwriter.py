...
def manageFinished(netManager, zipFileName, folder, progress):
    ...
    if not result.ok:
        ...
        global __appSDKification_doAgain
        if ('Session expired or invalid' in result.reason) and (not __appSDKification_doAgain):
            try:
...
def appSDKification(folder, progress):
    ...
    try:
        ...
        if __appSDKification_doAgain:
            QgsMessageLog.logMessage("Force to renew due to server expiration error", level=QgsMessageLog.WARNING)
